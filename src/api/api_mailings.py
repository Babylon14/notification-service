...
@router.post(path="/", response_model=MailingRead, status_code=status.HTTP_201_CREATED)
async def create_and_start_mailing(
    ...
    new_mailing = Mailing(
        ...
        owner_id=current_user.id # ID авторизованного пользователя
    )
    # 2. Добавляем в базу и сохраняем
    db.add(new_mailing)
    await db.commit()
...
@router.get(path="/me", response_model=List[MailingRead])
async def get_my_mailings(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user) # Только авторизованные пользователи
    ) -> List[MailingRead]:
    """Получение рассылок текущего пользователя"""

    query = select(Mailing).where(Mailing.owner_id == current_user.id)
